...
def _compute_counts(event, time, order=None):
    ...
    n_risks = 0 if (event.dtype == bool) else event.max()
...
def km_cumulative_incidence_competing_risks(
    ...
    Parameters
    ----------
    event : array-like, shape = (n_samples, n_risks+1)
        Contains binary event indicators.
    ...
    >>> x, y = km_ci_competing_risks(event, time)
    ...
    n_risks = event.max()
    uniq_times, n_events_cr, n_at_risk, n_censored = _compute_counts(event, time_exit)
    ...
    for i in range(1,n_risks+1):
        kpe_cum_inc[i] = np.cumsum(ratio[:,i]*kpe)
